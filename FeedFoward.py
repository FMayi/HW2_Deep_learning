...
class SigmoidCrossEntropy:
    ...
    # TODO: Compute the gradient of the cross entropy loss with respect to the the input logits
        ...
        if np.isnan(self.sigmoid).any():
            ...
            exit()
        self.loss = - ((labels * np.log(np.clip(self.sigmoid,1e-7, 1 - 1e-7))) + (1-labels) * (np.log(np.clip(1- self.sigmoid,1e-7, 1 - 1e-7))))
...
def evaluate(model, X_val, Y_val, batch_size):
    ...
    for i in range(0,num_examples,batch_size):
        ...
        batch_loss = -np.mean(y_batch * np.log(prediction) + (1 - y_batch)*np.log(1 - prediction))
...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        losses.append(epoch_loss / num_batches) # Average loss per epoch
        val_loss, val_acc = evaluate(net, X_test, Y_test, batch_size)
        val_losses.append(val_loss)
        val_accs.append(val_acc)
        epoch_avg_acc = np.mean((net.forward(X_train) > 0.5) == Y_train)
